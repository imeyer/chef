...
class Application < Merb::Controller
  ...
  def is_admin
    ...
    if @user.admin
      Chef::Log.debug("Authorized #{session[:user]} as Administrator")
      true
    else
      Chef::Log.debug("#{session[:user]} is not authorized as an administrator to run #{action_name}")
